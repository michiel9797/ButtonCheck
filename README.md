...
- Emulation programs must start by providing a handshake to the standard commandline:
```bash
EMULATE START
```
- Afterwards, emulation programs must only output in-order gamestates one line at a time to the standard commandline.
...
- Simulation programs must only output in-order gamestates one line at a time to the standard commandline.
